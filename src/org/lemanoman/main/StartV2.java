...
public class StartV2 {
	...
	String defaultImagemRepoLink = "http://centraldosquadrinhos.com/the-walking-dead/" + capitulo + "/";
	...
	try {
		...
		String tipo = "jpg";
		String link = "http://centraldosquadrinhos.com/wp-content/manga/Quadrinhos/thewalkingdead/edicao0"
			+ capitulo + "/the-walking-dead-" + getNumber(i) + "." + tipo;
		if(existsUrl(link)){
		    downloadCap(link, capDir, client, i + "." + tipo);
		}else{
		    tipo = "png";
		    link = "http://centraldosquadrinhos.com/wp-content/manga/Quadrinhos/thewalkingdead/edicao0"
				+ capitulo + "/the-walking-dead-" + getNumber(i) + "." + tipo;
		    if(existsUrl(link)){
			downloadCap(link, capDir, client, i + "." + tipo);
		    }else{
	    ...
	    /*
	     * if(e.text().contains(".jpg")){ String image = e.text(); String
	     * destinationFile = capDir.getPath()+"/"+image; if(!new
	     * File(destinationFile).exists()){ String url =
	     * defaultImagemRepoLink+image; url=url.replace(" ","%20"); HttpGet
	     * get = new HttpGet(url); get.setHeader("User-Agent",
	     * " Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:36.0) Gecko/20100101 Firefox/36.0"
	     * ); get.setHeader("Cookie",
	     * "__cfduid=da015e4ef87311ec5b9b85383c504587f1424188606; popup_user_login=yes"
	     * ); HttpResponse response = client.execute(get); System.out.print(
	     * "Searching : " +url+"\n"); System.out.print("Response Code : "
	     * +response.getStatusLine().getStatusCode()+"\n");
	     * download(response.getEntity(),destinationFile); }else{
    ...
    public String getNumber(String numStr) {
    ...
    public void downloadCap(String imageLink, File capDir, HttpClient client, String image) {
	try {
